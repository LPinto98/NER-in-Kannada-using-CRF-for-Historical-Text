...
filepath = os.path.join(BASE_DIR,'main/kannada_training.txt')
...
testpath = sys.argv[1]
ner = []
file_test = open(testpath, 'r', encoding='utf-8')
test_input =[]
nl=''
l=[]
...
file = open(os.path.join(BASE_DIR,"main/intermediate.txt"), mode = 'w',encoding = 'utf-8')
...
for j in range(len(test_input)):
        ...
        #file_output = open("intermediate.txt", 'w', encoding='utf-8')
       
        for i, x in enumerate(pos_tags):
                file.write(test_words[i] + "\t" + tags[x]+ "\n")
        file.write('.\tSYM\n')        
...
'''file_output = open("kanrosh_tags.txt", 'a', encoding='utf-8')
for i in range(len(test_words)):
        print(str(test_words[i]) + "\t" + str(pos_tags[i]) + "\t" + str(nery[i]))
        file_output.write(str(test_words[i]) + "\t" + str(pos_tags[i]) + "\t" + str(nery[i]))'''
